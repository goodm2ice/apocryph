...
class JobController:
    ...
    def __add_job(self, job_key: str, schedule: CombinedJob | IntervalJob | CalendarIntervalJob | DateJob | CronJob | None = None):
        ...
        self.jobs[job_key] = jobs[job_key](schedule)
    ...
    async def load(self):
        ...
        try:
            async with Session() as session:
                schedules = await session.exec(select(JobSchedule))
                for s in schedules:
                    if s.job_key not in jobs:
                        # TODO: Вывести в логер предупреждение
                        continue
                    self.__add_job(s.job_key, s.schedule)
        except Exception:
            pass
    ...
    async def save_all(self):
        ...
        async with Session() as session:
            records = list(await session.exec(select(JobSchedule)))
            ...
            for key, job in self.jobs.items():
                if key not in keys:
                    records.append(JobSchedule(job_key=key, schedule=job.schedule))
    ...
    async def set_schedule(self, job_key: str, schedule: CombinedJob | IntervalJob | CalendarIntervalJob | DateJob | CronJob):
        ...
        async with Session() as session:
            record = await session.get(JobSchedule, job_key)
            if record:
                record.schedule = schedule
            else:
                record = JobSchedule(job_key=job_key, schedule=schedule)
    ...
    async def run_now(self, job_key: str):
        ...
        if job_key not in self.jobs:
            return self.__add_job(job_key=job_key, trigger=DateJob(date=datetime.now()))
